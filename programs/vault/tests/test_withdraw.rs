...
fn test_withdraw() {
    ...
    // Creating "withdraw" instruction
    let vault_withdraw_instruction = Instruction::new_with_bytes(
        program_id,
        &vault::instruction::Withdraw {amount: 500_000_000}.data(),
    ...
    let blockhash = svm.latest_blockhash();
    let msg = Message::new_with_blockhash(&[vault_withdraw_instruction], Some(&payer.pubkey()), &blockhash);
    let tx = VersionedTransaction::try_new(VersionedMessage::Legacy(msg), &[&payer]).unwrap();
    ...
    assert_eq!(vault_balance_after, balance_before);
